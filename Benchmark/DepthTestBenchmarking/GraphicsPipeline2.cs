...
public sealed class GraphicsPipeline2<TVIn, TFIn> where TVIn : unmanaged where TFIn : unmanaged
{
    ...
    public void Render(IReadOnlyList<TVIn> vertices, IReadOnlyList<int> indices, IRenderTarget2 renderTarget2)
    {
        _renderTarget2 = renderTarget2;
        _vertices = vertices;
        _indices = indices;

        _viewport.Set(renderTarget2.Width, renderTarget2.Height);
    ...
    private void ProcessFragment(Fragment<TFIn> fragment)
    {
        ...
        if (!_renderTarget2.ZTest(fragment.Position.X, fragment.Position.Y, fragment.Position.Z))
            return;

        _fragmentShader.ProcessFragment(in fragment.Position, in fragment.Data, out Vector4 color);
        _renderTarget2.SetPixel(fragment.Position.X, fragment.Position.Y, fragment.Position.Z, in color);
    }
